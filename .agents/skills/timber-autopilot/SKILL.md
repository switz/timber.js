---
name: timber-autopilot
description: Run the full timber.js task-to-merge autopilot. Picks an lb task (or uses a provided task ID), reads design docs, implements with tests, self-reviews, and merges. Use when asked to "run autopilot", "pick up a task", or "work on the next issue".
---

# Timber Autopilot

Run the full timber.js task-to-merge autopilot for an lb task. If arguments are provided, use that task ID. Otherwise pick the next ready task.

## Phase 0: Select and claim a task

If no task ID is provided, run `lb ready --json` and pick the highest-priority unowned task. If a task ID is given, use it directly. Claim it with `lb update <id> --status in_progress --json`. If you decide to work on a task yourself, claim it so no other streams grab it.
...
Ask questions if we need to clarify anything.
