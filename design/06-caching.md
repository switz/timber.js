...
- **`MemoryCacheHandler`** (default) — in-process LRU. Each server instance has its own isolated cache. Invalidation on one instance does not affect others. `revalidateTag` only clears the current instance's cache. Acceptable for single-instance deployments; use a shared handler for multi-instance.
- **`RedisCacheHandler`** — shared cache. All instances call the same Redis; `invalidate()` removes the entry from the shared store. All instances see the invalidation on their next cache lookup. Redis pub/sub can be layered on top for push notification, but the cache handler itself guarantees correctness via the shared store.
