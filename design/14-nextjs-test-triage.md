...
- Multi-value searchParams (`?tag=a&tag=b`) returns last value instead of array (note: multi-value *headers* like `Vary` and `Set-Cookie` were fixed in PR #291, but query param parsing is a separate issue)
