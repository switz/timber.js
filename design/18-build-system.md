...
All mutable module-level state that requires singleton semantics is centralized in dedicated registry files. This makes the invariant explicit, prevents accidental state duplication, and is enforced by `tests/singleton-audit.test.ts`.
...
**Rule: Do not create `new AsyncLocalStorage()` outside `als-registry.ts`** (except for the documented exceptions). The `singleton-audit.test.ts` enforces this statically.
