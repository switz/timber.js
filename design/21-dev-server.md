...
| 10  | Server-Timing header: detailed in dev, safe in production | In dev mode, emits detailed per-phase `Server-Timing` breakdown (proxy, middleware, render) via ALS-based collector. In production, emits only `total;dur=N` — no phase names, preventing information disclosure while giving browser DevTools useful timing data. |
