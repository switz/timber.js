...
The build-time codegen already emits a `Routes` type map with `searchParams` per route. We extend it to emit `useQueryStates` overloads in the `@timber/app/client` module augmentation — the same pattern used for `useParams`:

```typescript
// Generated by codegen
declare module '@timber/app/client' {
