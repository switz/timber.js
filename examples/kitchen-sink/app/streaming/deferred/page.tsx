  ...
  return (
    <div data-testid="deferred-page" className="max-w-2xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-stone-500">
          <code className="rounded bg-stone-100 px-1 py-0.5 text-xs font-mono">
            deferSuspenseFor = 500ms
          </code>{' '}
          holds the stream so fast-resolving boundaries render inline without a loading flash.
        </p>
      </div>
      ...
      <Suspense
        fallback={
          <div
            data-testid="deferred-fast-fallback"
      ...
      >
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4">
          <div className="text-xs font-medium text-emerald-600 mb-1">
            Fast child (50ms &lt; 500ms deadline)
          </div>
      ...
      <Suspense
        fallback={
          <div
            data-testid="deferred-slow-fallback"
            className="rounded-lg border border-dashed border-stone-300 bg-stone-50 p-4 text-sm text-stone-500"
          >
            Slow loading...
          </div>
      ...
      <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
        <p className="text-sm text-amber-800">
          The fast child resolves within the hold window and renders inline (no fallback flash). The
          slow child exceeds the deadline — its fallback shows, then content streams in.
        </p>
      </div>
    </div>
  );
