...
The default is in-memory. Plug in Redis, Cloudflare KV, or any custom handler that implements the `CacheHandler` interface. See [Caching](/pre-release/docs/v1/caching).
...
For the full type definition, see the [Config API Reference](/pre-release/docs/v1/api-config).
