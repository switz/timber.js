...
import { deny, redirect, DenySignal, RedirectSignal } from '../packages/timber-app/src/server/primitives'
import type { AccessGateProps, SlotAccessGateProps } from '../packages/timber-app/src/server/tree-builder'
...
describe('AccessGate', () => {
  ...
  it('shallowest failure wins', async () => {
    ...
    await expect(AccessGate(makeAccessGateProps({ accessFn: parentAccessFn }))).rejects.toThrow(DenySignal)
  ...
  it('segment deny status', async () => {
    ...
    // deny(401)
    const accessFn401 = async () => { deny(401) }
    try {
    ...
    // deny(404)
    const accessFn404 = async () => { deny(404) }
    try {
  ...
  it('propagates redirect signal', async () => {
    const accessFn = async () => { redirect('/login') }
  ...
  it('propagates redirect with custom status', async () => {
    const accessFn = async () => { redirect('/new-location', 301) }
  ...
  it('propagates unhandled errors', async () => {
    ...
    await expect(AccessGate(makeAccessGateProps({ accessFn }))).rejects.toThrow('Database connection failed')
  })
})
...
describe('SlotAccessGate', () => {
  ...
  it('parent blocks slots — parent segment failure prevents slot rendering', async () => {
    ...
    const parentAccessFn = async () => { deny(401) }

    // Parent AccessGate throws — child slots never render
    await expect(
      AccessGate(makeAccessGateProps({ accessFn: parentAccessFn }))
  ...
  it('re-throws unhandled errors for error boundaries', async () => {
    ...
    await expect(
      SlotAccessGate(makeSlotAccessGateProps({ accessFn }))
