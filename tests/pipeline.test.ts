...
import { createPipeline, type PipelineConfig, type RouteMatch } from '../packages/timber-app/src/server/pipeline'
import { runProxy, type ProxyFn } from '../packages/timber-app/src/server/proxy'
import { runMiddleware, type MiddlewareFn } from '../packages/timber-app/src/server/middleware-runner'
...
describe('runProxy()', () => {
  ...
  it('array form composes left-to-right', async () => {
    const order: string[] = []
    const a: ProxyFn = async (_req, next) => { order.push('a'); return next() }
    const b: ProxyFn = async (_req, next) => { order.push('b'); return next() }
