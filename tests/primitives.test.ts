...
describe('redirectExternal()', () => {
  it('throws RedirectSignal for allowed URLs', () => {
    expect(() => redirectExternal('https://example.com/callback', ['example.com'])).toThrow(
      RedirectSignal
    );
  });

  it('rejects URLs not in the allow-list', () => {
    expect(() => redirectExternal('https://evil.com/phish', ['example.com'])).toThrow(
      /not in the allow/
    );
  });
  ...
  it('rejects with empty allow-list', () => {
    expect(() => redirectExternal('https://example.com', [])).toThrow(/not in the allow/);
  });
...
describe('waitUntil()', () => {
  ...
  it('warns once when adapter does not support waitUntil', () => {
    ...
    const timberWarns = warnSpy.mock.calls.filter((c) => String(c[0]).includes('waitUntil'));
