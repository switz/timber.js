...
describe('singleflight', () => {
  ...
  it('propagates errors to all waiters', async () => {
    ...
    const results = await Promise.allSettled([sf.do('key', work), sf.do('key', work)]);
...
describe('timber.cache', () => {
  ...
  it('tags as function of args', async () => {
    ...
    const cached = createCache(fn, { ttl: 60, tags: (id: string) => [`user:${id}`] }, handler);
  ...
  it('custom key function', async () => {
    const fn = vi.fn(async (id: string) => ({ id }));
    const cached = createCache(fn, { ttl: 60, key: (id: string) => `custom:${id}` }, handler);
  ...
  it('singleflight: concurrent misses produce single execution', async () => {
    ...
    const [a, b, c] = await Promise.all([cached('abc'), cached('abc'), cached('abc')]);
  ...
  it('invalidate by key', async () => {
    const fn = vi.fn(async () => 'value');
    const cached = createCache(fn, { ttl: 60, key: () => 'my-key' }, handler);
    const invalidate = createCache.invalidate.bind(null, handler);
