...
function PageComponent(props: unknown) { return { page: true, props } }
function LayoutComponent(props: unknown) { return { layout: true, props } }
function RootLayoutComponent(props: unknown) { return { rootLayout: true, props } }
function ErrorComponent(props: unknown) { return { error: true, props } }
function NotFoundComponent(props: unknown) { return { notFound: true, props } }
function AccessFn(ctx: unknown) { return ctx }
function SlotPageComponent(props: unknown) { return { slotPage: true, props } }
function SlotDeniedComponent(props: unknown) { return { slotDenied: true, props } }
function SlotDefaultComponent(props: unknown) { return { slotDefault: true, props } }
function SlotAccessFn(ctx: unknown) { return ctx }
...
describe('bottom-up tree construction', () => {
  it('builds a tree with page only', async () => {
    ...
    const result = await buildElementTree(makeConfig({
      segments,
...
describe('edge cases', () => {
  it('throws on empty segment chain', async () => {
    await expect(
      buildElementTree(makeConfig({ segments: [] }))
    ).rejects.toThrow('empty segment chain')
  })
  ...
  it('throws when leaf has no page.tsx and no route.ts', async () => {
    ...
    await expect(
      buildElementTree(makeConfig({ segments }))
    ).rejects.toThrow('No page component found')
  })
